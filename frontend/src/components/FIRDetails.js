...
import LinkCriminalForm from './LinkCriminalForm';
...
function FIRDetails() {
  ...
  // Helper to render evidence item based on file extension
  const renderEvidence = (ev) => {
    ...
    if (extension === 'mp4' || extension === 'webm' || extension === 'ogg') {
      return (
        <video controls className="w-full max-w-md rounded-lg shadow">
      ...
      return (
        <img 
          src={fullUrl} 
          alt={ev.description} 
          className="max-w-full max-h-64 rounded-lg shadow object-contain"
        />
      );
    } else {
      return (
        ...
        >
          Download {ev.description} ({extension})
        </a>
      );
    }
  };
